...
export default async function EditDocumentPage({
  ...
  const [document, categories, products] =
    await Promise.all([
  ...
  const toEmails = document.recipients
    .filter(
      (recipient) =>
        recipient.type === "TO"
    )
  ...
  const ccEmails = document.recipients
    .filter(
      (recipient) =>
        recipient.type === "CC"
    )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Edit {document.documentNumber}. Saving changes will
